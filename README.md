...
`tutorials/mitra_regressor_colab.ipynb` (`E2E`) and its companion `tutorials/mitra_regressor_predictor_inference_colab.ipynb` (`ARTIFACT-INFERENCE`) are declared under DIMER Notebook Specification 1.1 and are **standalone** (§3.6): generated by `tools/build_notebook.py`, each carries the public tutorial API module `mitra_pipeline/tutorial_api.py` verbatim, the model identity (`autogluon/mitra-regressor` at immutable revision `5f277aa8f69042d39d6ac3612aed18bb9279bd95`), the snapshot manifest digests (`model.safetensors` `d8e75c62af0bec2fd404b0ad20a442d951d43ca6d331315cfcc0509b54f2c642`, `config.json` `2bc1ed5047f7c25368245e8ad32540a5fa28940b1ec05d3f1f454a09ff5384c1`) and the exact `pyproject.toml` runtime pins, so the exported notebooks run without this repository or DIMER Workbench (parity enforced by `tests/test_notebook_parity.py`, `tests/test_companion_parity.py` and `tools/validate_release_assets.py`). The E2E default path loads scikit-learn's bundled diabetes table (no download), validates it with `validate_inputs` into an input manifest, registers the support rows through AutoGluon, evaluates on a holdout and an independent test partition against `training_mean_baseline`, Dummy mean/median, LightGBM and Random Forest, writes an `evaluation_report` (`sample-sanity`), optionally fine-tunes on a GPU (`RUN_FINE_TUNING`, off by default) and optionally scores an uploaded CSV, exports the deployable predictor bundle with its manifest and provenance and proves it reloads from a fresh directory. The companion consumes a bundle from a separate execution (upload or `ARTIFACT_ZIP_PATH`, trusted whole-archive digest required), validates it before any deserialisation, validates new unlabelled rows into an input manifest, predicts, and writes a `not-measurable` evaluation report. The repository's `examples/sample-data/` archives are used through the pre-split upload path. See `tutorials/README.md` for the registry, `docs/WEIGHTS.md` for the snapshot layout and `docs/release-verification.md` for the release gate.

## Release status

**Candidate.** Static/unit checks do not constitute clean-runtime notebook evidence. The previous repository-installing pair was executed by the release workflow in a clean hosted runner; the standalone pair has not been executed yet. Complete `docs/release-verification.md` against the exact release revision before calling either notebook release-grade.
